      ...
      if (searchResult) {
        ...
        if (!inventoryCategory) {
          ...
          if (!inventoryItem) {
            ...
            inventoryCategory.items.push({ itemId: item.itemId, name: item.name, quantity: item.quantity, paidAmount: Math.round(Number(item.quantity))  });
          } else {
            inventoryItem.quantity = Number(inventoryItem.quantity) + Math.round(Number(item.quantity));
            inventoryItem.paidAmount = Number(inventoryItem.paidAmount) + Math.round(Number(item.quantity));
          }
        }
      } else {
